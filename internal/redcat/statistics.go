...
func (s *StatsStore) AddPoints(streamer string, reason PointReason, points int, at time.Time) error {
	if points <= 0 || streamer == "" || reason == "" { return nil }
	s.mu.Lock()
	defer s.mu.Unlock()
...
func cloneReason(in map[PointReason]int64) map[PointReason]int64 {
	out := make(map[PointReason]int64, len(in))
	for k, v := range in { out[k] = v }
	return out
}

func cloneString(in map[string]int64) map[string]int64 {
